...
var EventEmitter, Promise, Zookeeper,
...
Zookeeper = (function(_super) {
  ...
  function Zookeeper(handler, options) {
    ...
    this.socket.on('connect', (function(_this) {
      return function() {
        return _this.getRegisterMessage().then(function(registerMessage) {
          Zookeeper.logger.info('[zookeeper] connected, registering:', JSON.stringify(registerMessage));
          _this.socket.emit('register', registerMessage);
          return _this.handler.on('heartbeat', function(message) {
            var _base;
            if (typeof (_base = Zookeeper.logger).debug === "function") {
              _base.debug('[zookeeper] heartbeat:', message);
            }
            return _this.socket.emit('heartbeat', message);
          });
        })["catch"](function(err) {
          return Zookeeper.logger.error('[zookeeper] error registering:', err.stack || err);
        });
      };
    })(this));
  }

  Zookeeper.prototype.getRegisterMessage = function() {
    var getVersion;
    getVersion = (function(_this) {
      return function() {
        var _base;
        if (_this.options.version) {
          return Promise.resolve(_this.options.version);
        }
        if (typeof (_base = Zookeeper.logger).debug === "function") {
          _base.debug('[zookeeper] no version specified, using git commit id instead');
        }
        return (require('git-info'))().then(function(git) {
          return git.shortCommitId;
        });
      };
    })(this);
    return getVersion().then((function(_this) {
      return function(version) {
        return {
          component: _this.options.component,
          host: _this.options.host || require('os').hostname().toLowerCase(),
          version: version
        };
      };
    })(this));
